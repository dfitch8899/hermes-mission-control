    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <MetricCard
            label="Agent Status"
            value={<span className="flex items-center gap-2"><span className="w-3 h-3 rounded-full inline-block animate-live-pulse" style={{ backgroundColor: '#3FB950' }} />ONLINE</span>}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <span className="text-[10px] font-mono uppercase tracking-widest" style={{ color: '#8B949E' }}>
                Recent Tasks
              </span>
              <a href="/tasks" className="text-[10px] font-mono uppercase tracking-widest transition-colors duration-100" style={{ color: '#484F58' }}
              ...
              {recentTasks.map((task, i) => (
                <div
                  ...
                  style={{
                    ...
                    borderBottom: i < recentTasks.length - 1 ? '0.5px solid #21262D' : 'none',
                  }}
                >
                  <div
                    ...
                    style={{
                      backgroundColor:
                        task.status === 'in_progress' ? '#FFB300'
                        : task.status === 'done' ? '#3FB950'
                        : task.status === 'queued' ? '#388BFD'
                        : '#484F58',
                    }}
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-medium truncate" style={{ color: '#E6EDF3' }}>{task.title}</p>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <a href="/memory" className="text-[10px] font-mono uppercase tracking-widest transition-colors duration-100" style={{ color: '#484F58' }}
                onMouseEnter={(e) => { e.currentTarget.style.color = '#FFB300' }}
                onMouseLeave={(e) => { e.currentTarget.style.color = '#484F58' }}
              >
                View all →
              ...
              {recentMemories.map((mem, i) => (
                <div
                  ...
                  style={{
                    ...
                    borderBottom: i < recentMemories.length - 1 ? '0.5px solid #21262D' : 'none',
                    paddingBottom: i < recentMemories.length - 1 ? '12px' : '0',
                  }}
                >
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <p className="text-[12px] font-medium leading-snug" style={{ color: '#E6EDF3' }}>{mem.title}</p>
                    <Badge variant={mem.type === 'context' ? 'blue' : mem.type === 'skill' ? 'amber' : 'teal'}>
                      {mem.type}
                    </Badge>
                  </div>
                  <p className="text-[11px] line-clamp-2" style={{ color: '#8B949E' }}>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <a href="/calendar" className="text-[10px] font-mono uppercase tracking-widest transition-colors duration-100" style={{ color: '#484F58' }}
                onMouseEnter={(e) => { e.currentTarget.style.color = '#FFB300' }}
                onMouseLeave={(e) => { e.currentTarget.style.color = '#484F58' }}
              >
                View all →
              ...
              {upcomingEvents.map((evt, i) => (
                <div
                  ...
                  style={{
                    ...
                    borderBottom: i < upcomingEvents.length - 1 ? '0.5px solid #21262D' : 'none',
                ...
                >
                  ...
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-medium truncate" style={{ color: '#E6EDF3' }}>{evt.title}</p>
                    <p className="text-[10px] font-mono mt-0.5" style={{ color: '#484F58' }}>
                      <Countdown target={evt.nextRun} />
                    </p>
                    {evt.cronExpression && (
                      <p className="text-[10px] font-mono" style={{ color: '#484F58' }}>
                        {evt.cronExpression}
                      </p>
                    )}
                  </div>
